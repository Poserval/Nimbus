...
let folderPath = [{ id: 'root', name: 'Корень' }];
let activeProgress = {};
...
function renderItemsList(items) {
    ...
    if (currentView === 'list') {
        ...
        for (const item of sorted) {
            ...
            let folderInfo = '';
            if (isFolder && window.folderFileCounts && window.folderFileCounts[item.id]) {
                folderInfo = ` • ${window.folderFileCounts[item.id]} файлов`;
            }
        ...
        for (const item of sorted) {
            ...
            let folderInfo = '';
            if (isFolder && window.folderFileCounts && window.folderFileCounts[item.id]) {
                folderInfo = `<div class="folder-info">${window.folderFileCounts[item.id]} файлов</div>`;
            }
...
function attachItemEvents() {
    ...
    document.querySelectorAll('.file-click-area').forEach(area => {
        area.addEventListener('click', async (e) => {
            ...
            if (isFolder) {
                await enterFolder(itemId, fileName);
...
async function initRealLimit() {
    try {
        if (typeof getRealLimit === 'undefined') {
            console.warn('getRealLimit not available, using default 15GB');
            limitGB = 15;
            document.getElementById('limitInput').value = 15;
            await updateStats();
            return;
        }
        
        const realLimit = await getRealLimit('google', accessToken);
        if (realLimit) {
            limitGB = realLimit;
            document.getElementById('limitInput').value = realLimit.toFixed(1);
            await updateStats();
            console.log(`Real limit set to ${realLimit} GB`);
        } else {
            limitGB = 15;
            document.getElementById('limitInput').value = 15;
            await updateStats();
        ...
        console.error('Failed to get real limit:', err);
        limitGB = 15;
        document.getElementById('limitInput').value = 15;
        await updateStats();
    }
...
document.getElementById('renameBtn').onclick = () => {
    ...
    renameItem(itemId, oldName, newFullName, isFolder);
};
...
document.getElementById('createFolderConfirmBtn').onclick = async () => {
    ...
    try {
        const newFolderId = await createFolderInCurrent(newFolderName, currentFolderId);
        alert(`Папка "${newFolderName}" создана!`);
